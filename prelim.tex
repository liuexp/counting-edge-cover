...
We use $0$ to denote scalar value $0$, and $\mathbf{0}$ to denote all-zero vector, and $\set{e_i}_{i=1}^{d}$ denote the $d$-dimensional vector with $i$-th coordinate being $e_i$, so $\set{e_i} = \mathbf{0}$ means $\forall i, e_i = 0$.
