...
\begin{frame}
	\frametitle{Devising sub-problems} % Here we introduce the only concept that you need to keep in mind.
...
\begin{itemize}
  \item FPTAS for weighted edge covers. (Arbitrary edge weights).
  \item FPTAS for Read-5-Mon-CNF, while Read-6-Mon-CNF does not admit FPTAS unless $P=NP$.
  \item FPTAS for hypergraph matching in certain range (e.g. $3$D-Matching with maximum degree $4$).
  \item \dots
\end{itemize}
\end{frame}
\begin{frame}
